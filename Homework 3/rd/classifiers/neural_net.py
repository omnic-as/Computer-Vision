...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    loss = -1.0 / N * np.sum(np.log(softmax_output[range(N), list(y)]))
    ...
    grads['W1'] = 1.0 / N * X.T.dot(dh1) + reg * W1
    grads['b1'] = 1.0 / N * np.sum(dh1, axis=0)
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in range(num_iters):
      ...
      # Use the gradients in `grads` to update the parameters
